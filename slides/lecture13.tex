...
\hlstd{random_sample} \hlkwb{<-} \hlkwd{sample}\hlstd{(height,} \hlnum{5}\hlstd{,} \hlkwc{replace} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\hlstd{random_sample}
\end{alltt}
\begin{verbatim}
## [1] 65 75 69 71 60
...
\hlstd{draw_xbar} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{n}\hlstd{) \{}
  \hlstd{random_sample} \hlkwb{<-} \hlkwd{sample}\hlstd{(height,} \hlkwc{size} \hlstd{= n,} \hlkwc{replace} \hlstd{=} \hlnum{FALSE}\hlstd{)}
  \hlkwd{mean}\hlstd{(random_sample)}
\hlstd{\}}
