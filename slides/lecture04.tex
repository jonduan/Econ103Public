...
\includegraphics[width=\maxwidth]{figure/plot_exams-1} 
...
\hlstd{exams} \hlkwb{<-} \hlkwd{read.csv}\hlstd{(data_url)}

\hlcom{# Drop students who missed an exam}
\hlstd{exams} \hlkwb{<-} \hlkwd{na.omit}\hlstd{(exams)}

\hlcom{# Run the regression and display the slope and intercept}
\hlstd{reg} \hlkwb{<-} \hlkwd{lm}\hlstd{(Midterm2} \hlopt{~} \hlstd{Midterm1,} \hlkwc{data} \hlstd{= exams)}
\hlkwd{coef}\hlstd{(reg)}
\end{alltt}
\begin{verbatim}
## (Intercept)    Midterm1 
...
\begin{alltt}
\hlcom{# By hand}
\hlnum{32.5745441} \hlopt{+} \hlnum{0.6130357} \hlopt{*} \hlnum{89}
\end{alltt}
\begin{verbatim}
## [1] 87.13472
\end{verbatim}
\begin{alltt}
\hlcom{# Using predict()}
\hlstd{missing_student} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlkwc{Midterm1} \hlstd{=} \hlnum{89}\hlstd{)}
\hlkwd{predict}\hlstd{(reg,} \hlkwc{newdata} \hlstd{= missing_student)}
\end{alltt}
\begin{verbatim}
##        1 
## 87.13472
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{x} \hlkwb{<-} \hlkwd{seq}\hlstd{(}\hlkwc{from} \hlstd{=} \hlopt{-}\hlnum{1}\hlstd{,} \hlkwc{to} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{by} \hlstd{=} \hlnum{0.1}\hlstd{)}
...
\end{verbatim}
\begin{alltt}
\hlkwd{plot}\hlstd{(x,y);} \hlkwd{abline}\hlstd{(}\hlkwd{lm}\hlstd{(y} \hlopt{~} \hlstd{x))}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/quadratic_plot-1} 
